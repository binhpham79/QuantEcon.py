...
def lemke_howson(g, init_pivot=0, max_iter=1000):
    try:
        N = g.N
    except:
...
def initialize_tableaux(payoff_matrices, tableaux, bases):
    ...
    payoff_mins = (payoff_matrices[0].min(), payoff_matrices[1].min())
        ...
        for i in range(nums_actions[1-pl]):
            for j in range(nums_actions[pl]):
                tableaux[pl][i, py_start+j] = \
                    payoff_matrices[1-pl][i, j] + payoff_mins[1-pl] + 1
